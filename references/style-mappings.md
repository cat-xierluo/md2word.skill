...
连续的 `>` 行只按 Markdown 语义进入同一个 `quote` 渲染器，不识别“本章导读”“案例”等内容标签。每一段直接写入段落底纹，不创建 `w:tbl`；首/尾段分别承担上下内边距，每段共享左右内边距，底纹与正文左右边界齐平。脚注、粗体、斜体、列表 marker 和行内代码继续沿用原有行内格式解析。完整字段见 [config-reference.md](config-reference.md#引用块格式-quote)。
