...
| `> 引用` | 单单元格 callout 表格 | 与正文同宽、无边框灰底；导读和案例共用 `quote` 配置 |
...
连续的 `>` 行只按 Markdown 语义进入同一个 `quote` 渲染器，不识别“本章导读”“案例”等内容标签。容器宽度、底纹、边框、内边距和前后间距均由 `quote` 配置控制；正文内的脚注、粗体、斜体和行内代码继续沿用原有行内格式解析。完整字段见 [config-reference.md](config-reference.md#引用块格式-quote)。
